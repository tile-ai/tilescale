...
// Use the CUDA Toolkit's public Driver API declarations. The build already
// requires CUDAToolkit when the CUDA backend is enabled and propagates its
// include directory to this target.
#include <cuda.h>

// Symbol visibility macros for shared library export.
// On Windows, the CUDA Toolkit header already declares these functions (with
// CUDAAPI calling convention). Adding __declspec(dllexport) would cause C2375
...
//
// On Windows, the CUDA Toolkit header already declares all these functions
// (with CUDAAPI calling convention), so we skip the re-declarations to avoid
// C2375 linkage conflicts. On POSIX, we need them for
