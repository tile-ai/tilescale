...
// TileScale: Initialize distributed table by copying host data to device meta_data symbol
class TileScaleInitDistributedTable {
 public:
  ...
  TileScaleInitDistributedTable(TileScaleCUDAModuleNode* m, ffi::ObjectPtr<ffi::Object> sptr)
      : m_(m), sptr_(sptr) {
...
class TileScaleCUDAModuleNode : public ffi::ModuleObj {
 public:
  explicit TileScaleCUDAModuleNode(std::string data, std::string fmt,
...
void TileScaleInitDistributedTable::operator()(const ffi::PackedArgs& args, ffi::Any* rv) const {
...
class TileScaleCUDAWrappedFunc {
 public:
  ...
  void operator()(ffi::PackedArgs args, ffi::Any* rv, void** void_args) const {
    ...
    static auto get_stream = ffi::Function::GetGlobal("device_api.cuda.get_stream");
...
class TileScaleCUDAPrepGlobalBarrier {
 public:
  TileScaleCUDAPrepGlobalBarrier(TileScaleCUDAModuleNode* m, ffi::ObjectPtr<ffi::Object> sptr)
      : m_(m), sptr_(sptr) {
    std::fill(pcache_.begin(), pcache_.end(), 0);
  }
  ...
  void operator()(const ffi::PackedArgs& args, ffi::Any* rv) const {
    ...
    if (pcache_[device_id] == 0) {
      pcache_[device_id] =
          m_->GetGlobal(device_id, symbol::tvm_global_barrier_state, sizeof(unsigned));
    }
...
ffi::Optional<ffi::Function> TileScaleCUDAModuleNode::GetFunction(const ffi::String& name) {
