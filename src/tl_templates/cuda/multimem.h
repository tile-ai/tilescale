...
template <>
struct LdReduceV4<ReduceOp::ADD, float> {
...
template <>
struct LdReduceV4<ReduceOp::MIN, float> {
  TL_DEVICE static void run(void *dst, const void *mcast_ptr) {
    int4 ret;
    asm volatile(
...
template <>
struct LdReduceV4<ReduceOp::MAX, float> {
  TL_DEVICE static void run(void *dst, const void *mcast_ptr) {
    int4 ret;
    asm volatile(
...
struct StV4<float> {
  TL_DEVICE static void run(void *mcast_ptr, const void *src) {
    ...
    asm volatile(
        "multimem.st.relaxed.sys.global.v4.b32 [%0], {%1, %2, %3, %4};"
        :
...
template <>
struct RedV4<ReduceOp::ADD, float> {
  TL_DEVICE static void run(void *mcast_ptr, const void *src) {
    int4 val = *reinterpret_cast<const int4 *>(src);
    asm volatile(
...
template <>
struct RedV4<ReduceOp::MIN, float> {
  TL_DEVICE static void run(void *mcast_ptr, const void *src) {
...
template <>
struct RedV4<ReduceOp::MAX, float> {
  TL_DEVICE static void run(void *mcast_ptr, const void *src) {
    const float *src_f = reinterpret_cast<const float *>(src);
    const char *mc_bytes = reinterpret_cast<const char *>(mcast_ptr);
    #pragma unroll
