...
TL_DEVICE unsigned int sync_grids_arrive(uint32_t *barrier) {
  ...
  if (IS_MASTER_THREAD()) {
    ...
    asm volatile("atom.add.release.gpu.u32 %0,[%1],%2;" : "=r"(oldArrive) : "l"((unsigned int*)barrier), "r"(nb) : "memory");
  }
...
TL_DEVICE void sync_grids_wait(unsigned int oldArrive, uint32_t *barrier) {
  if (IS_MASTER_THREAD()) {
    ...
    do {
      asm volatile("ld.acquire.gpu.u32 %0,[%1];" : "=r"(current_arrive) : "l"((unsigned int *)barrier) : "memory");
    } while (!(((oldArrive ^ current_arrive) & 0x80000000) != 0));
  }
  __syncthreads();
}

TL_DEVICE void sync_grids(uint32_t *barrier) {
