...
struct StImpl {
  ...
  TL_DEVICE static void execute(T *ptr, T value) {
    static_assert(always_false_v<T>, 
                  "tl::st: unsupported configuration. ");
  }
};

template <Semantic semantic, Scope scope, bool nc, bool na>
struct LdImpl {
  template <typename T>
  TL_DEVICE static void execute(const T *ptr, T &value) {
    static_assert(always_false_v<T>, 
                  "tl::ld: unsupported configuration. ");
  }
};
...
#define TL_ST_IMPL(SEM, SCOPE, NA, SEM_LIT, SCOPE_LIT, NA_LIT) \
  template <> \
  struct StImpl<Semantic::SEM, Scope::SCOPE, NA> { \
    template <typename T> \
    TL_DEVICE static void execute(T *ptr, T value) { \
...
#define TL_LD_IMPL(SEM, SCOPE, NC, NA, SEM_LIT, SCOPE_LIT, NC_LIT, NA_LIT) \
  template <> \
  struct LdImpl<Semantic::SEM, Scope::SCOPE, NC, NA> { \
    template <typename T> \
    TL_DEVICE static void execute(const T *ptr, T &value) { \
      if constexpr (sizeof(T) == 2) { \
...
TL_LD_IMPL(WEAK, GPU, false, true, ".weak", ".gpu.global", "", ".L1::no_allocate")
TL_LD_IMPL(WEAK, SYS, false, true, ".weak", ".sys.global", "", ".L1::no_allocate")
...
TL_LD_IMPL(RELAXED, GPU, false, true, ".relaxed", ".gpu.global", "", ".L1::no_allocate")
TL_LD_IMPL(RELAXED, SYS, false, true, ".relaxed", ".sys.global", "", ".L1::no_allocate")
...
TL_LD_IMPL(ACQUIRE, GPU, false, true, ".acquire", ".gpu.global", "", ".L1::no_allocate")
TL_LD_IMPL(ACQUIRE, SYS, false, true, ".acquire", ".sys.global", "", ".L1::no_allocate")
