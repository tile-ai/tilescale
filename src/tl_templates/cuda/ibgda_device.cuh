...
//  - DeepEP/csrc/kernels/ibgda_device.cuh

// NOTE(wt): For now, we rely on NVSHMEM to implement IBGDA
...
template <typename T>
TL_DEVICE void store_relaxed_na(T* ptr, T value) {
    ::tl::st<Semantic::RELAXED, Scope::GPU, true>(ptr, value);
}

template <typename T>
TL_DEVICE void store_release_na(T* ptr, T value) {
    ::tl::st<Semantic::RELEASE, Scope::GPU, true>(ptr, value);
}

template <typename T>
TL_DEVICE T load_relaxed_na(const T* ptr) {
    T value;
    ::tl::ld<Semantic::RELAXED, Scope::GPU, false, true>(ptr, value);
    return value;
}

TL_DEVICE void store_relaxed_na_vec4(int4* ptr, const int4& value) {
...
TL_DEVICE uint64_t HtoBE64(uint64_t x) {
    uint64_t ret;
    asm("{\n\t"
        ".reg .b32 ign;\n\t"
        ".reg .b32 lo;\n\t"
...
TL_DEVICE uint32_t HtoBE32(uint32_t x) {
    uint32_t ret;
    asm("{\n\t"
        ".reg .b32 ign;\n\t"
...
TL_DEVICE uint16_t HtoBE16(uint16_t x) {
...
TL_DEVICE nvshmemi_ibgda_device_state_t* ibgda_get_state() {
    return &nvshmemi_ibgda_device_state_d;
}

TL_DEVICE nvshmemi_ibgda_device_qp_t* ibgda_get_rc(int pe, int id) {
...
TL_DEVICE void ibgda_lock_release(int* lock) {
    memory_fence_cta();

    // Prevent reordering before lock is released
    detail::store_relaxed_na(lock, 0);
}

TL_DEVICE void ibgda_update_dbr(nvshmemi_ibgda_device_qp_t* qp, uint32_t dbrec_head) {
    ...
    asm("{\n\t"
        ...
        : "=r"(dbrec_val)
        : "r"(dbrec_head));
    detail::store_release_na(dbrec_ptr, dbrec_val);
}

TL_DEVICE void ibgda_ring_db(nvshmemi_ibgda_device_qp_t* qp, uint16_t prod_idx) {
    ...
    static_assert(sizeof(decltype(&ctrl_seg)) == sizeof(uint64_t), "");
    detail::store_release_na(bf_ptr, *(reinterpret_cast<uint64_t*>(&ctrl_seg)));
}

TL_DEVICE void ibgda_post_send(nvshmemi_ibgda_device_qp_t* qp, uint64_t new_prod_idx) {
...
TL_DEVICE void ibgda_submit_requests(nvshmemi_ibgda_device_qp_t* qp,
...
TL_DEVICE void ibgda_write_rdma_write_inl_wqe(
    ...
    static_assert(sizeof(*ctrl_seg_ptr) == 16, "sizeof(*ctrl_seg_ptr) == 16");
    static_assert(sizeof(*raddr_seg_ptr) == 16, "sizeof(*raddr_seg_ptr) == 16");
    static_assert(sizeof(*inl_seg_ptr) == 4, "sizeof(*inl_seg_ptr) == 4");
    detail::store_relaxed_na_vec4(reinterpret_cast<int4*>(ctrl_seg_ptr), *reinterpret_cast<const int4*>(&ctrl_seg));
    detail::store_relaxed_na_vec4(reinterpret_cast<int4*>(raddr_seg_ptr), *reinterpret_cast<const int4*>(&raddr_seg));
    detail::store_relaxed_na(reinterpret_cast<uint32_t*>(inl_seg_ptr), *reinterpret_cast<const uint32_t*>(&inl_seg));
    detail::store_relaxed_na(reinterpret_cast<uint32_t*>(wqe_data_ptr), *reinterpret_cast<const uint32_t*>(val));
}
...
TL_DEVICE void ibgda_get_rkey(uint64_t addr, int dst_pe, uint64_t* out_raddr, __be32* out_rkey, uint32_t dev_idx) {
    auto state = ibgda_get_state();
    auto heap_start = reinterpret_cast<uint64_t>(nvshmemi_device_state_d.heap_base);
...
TL_DEVICE uint64_t ibgda_reserve_wqe_slots(nvshmemi_ibgda_device_qp_t* qp, uint32_t num_wqes) {
    auto mvars = &qp->mvars;
    return atomicAdd(reinterpret_cast<unsigned long long*>(&mvars->tx_wq.resv_head), static_cast<unsigned long long>(num_wqes));
}

TL_DEVICE void* ibgda_get_wqe_ptr(nvshmemi_ibgda_device_qp_t* qp, uint16_t wqe_idx) {
...
TL_DEVICE void nvshmemi_ibgda_rma_p(
...
TL_DEVICE void ibgda_write_rdma_write_wqe(nvshmemi_ibgda_device_qp_t* qp,
                                                                  uint64_t laddr,
                                                                  __be32 lkey,
                                                                  uint64_t raddr,
                                                                  __be32 rkey,
    ...
    static_assert(sizeof(*ctrl_seg_ptr) == 16, "sizeof(*ctrl_seg_ptr) == 16");
    static_assert(sizeof(*raddr_seg_ptr) == 16, "sizeof(*raddr_seg_ptr) == 16");
    static_assert(sizeof(*data_seg_ptr) == 16, "sizeof(*data_seg_ptr) == 16");
    detail::store_relaxed_na_vec4(reinterpret_cast<int4*>(ctrl_seg_ptr), *reinterpret_cast<const int4*>(&ctrl_seg));
    detail::store_relaxed_na_vec4(reinterpret_cast<int4*>(raddr_seg_ptr), *reinterpret_cast<const int4*>(&raddr_seg));
    detail::store_relaxed_na_vec4(reinterpret_cast<int4*>(data_seg_ptr), *reinterpret_cast<const int4*>(&data_seg));
}

TL_DEVICE void ibgda_write_empty_recv_wqe(void* out_wqe) {
    ...
    static_assert(sizeof(mlx5_wqe_data_seg) == sizeof(int4), "Invalid data type length");
    detail::store_relaxed_na_vec4(reinterpret_cast<int4*>(data_seg_ptr), *reinterpret_cast<const int4*>(&data_seg));
}

template <bool kAlwaysDoPostSend = false>
TL_DEVICE void nvshmemi_ibgda_put_nbi_warp(
    ...
    if (!(num_wqes <= 32)) {
        printf("Assertion failed: %s:%d, condition: num_wqes <= 32\n", __FILE__, __LINE__);
        trap();
    }
...
TL_DEVICE void ibgda_write_amo_add_wqe(nvshmemi_ibgda_device_qp_t* qp,
                                                               const int& value,
                                                               uint64_t laddr,
                                                               __be32 lkey,
                                                               uint64_t raddr,
    ...
    static_assert(sizeof(*ctrl_seg_ptr) == sizeof(int4), "Invalid vectorization");
    static_assert(sizeof(*raddr_seg_ptr) == sizeof(int4), "Invalid vectorization");
    static_assert(sizeof(*atomic_seg_ptr) == sizeof(int4), "Invalid vectorization");
    static_assert(sizeof(*data_seg_ptr) == sizeof(int4), "Invalid vectorization");
    detail::store_relaxed_na_vec4(reinterpret_cast<int4*>(ctrl_seg_ptr), *reinterpret_cast<int4*>(&ctrl_seg));
    detail::store_relaxed_na_vec4(reinterpret_cast<int4*>(raddr_seg_ptr), *reinterpret_cast<int4*>(&raddr_seg));
    detail::store_relaxed_na_vec4(reinterpret_cast<int4*>(atomic_seg_ptr), *reinterpret_cast<int4*>(&atomic_seg_1));
    detail::store_relaxed_na_vec4(reinterpret_cast<int4*>(data_seg_ptr), *reinterpret_cast<int4*>(&data_seg));
}

TL_DEVICE void nvshmemi_ibgda_amo_nonfetch_add(
...
TL_DEVICE uint64_t nvshmemi_get_p2p_ptr(const uint64_t& ptr, const int& rank, const int& dst_rank) {
...
TL_DEVICE void ibgda_poll_cq(nvshmemi_ibgda_device_cq_t* cq, uint64_t idx) {
    ...
    do {
        wqe_counter = HtoBE16(detail::load_relaxed_na(&cqe64->wqe_counter));
...
TL_DEVICE void nvshmemi_ibgda_quiet(int dst_pe, int qp_id) {
    auto qp = ibgda_get_rc(dst_pe, qp_id);
    auto state = ibgda_get_state();
