...
def combine(
        ...
        num_ranks: int = group_size) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
    """
    ...
    assert send_head.dim() == 2 and send_head.is_contiguous() and send_head.scalar_type(
    ) == torch.int32
    assert rank_prefix_matrix.dim() == 2 and rank_prefix_matrix.is_contiguous(
    ) and rank_prefix_matrix.scalar_type() == torch.int32
    assert channel_prefix_matrix.dim() == 2 and channel_prefix_matrix.is_contiguous(
    ) and channel_prefix_matrix.scalar_type() == torch.int32
    ...
    assert hidden * x.element_size() % (
        4 * torch.int32.itemsize()) == 0  # x's last dim can be split into int4
    ...
    ) and topk_weights.dtype == torch.float32 and topk_weights.size(0) == num_tokens
    ...
    assert bias_0.dim() == 2 and bias_0.is_contiguous(
    ) and bias_0.dtype == x.dtype and bias_0.shape == (num_recv_tokens, hidden)
    if bias_1 is not None:
        assert bias_1.dim() == 2 and bias_1.is_contiguous(
        ) and bias_1.dtype == x.dtype and bias_1.shape == (num_recv_tokens, hidden)
