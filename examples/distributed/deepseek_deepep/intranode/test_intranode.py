...
def test_intranode(
    ...
    ref_num_tokens_per_rank, _, ref_num_tokens_per_expert, ref_is_token_in_rank, _ = deepep_buffer.get_dispatch_layout(topk_idx, num_experts)
    ...
    # ours
    if cached_dispatch:
        recv_x = ts_buffer.dispatch(x, ref_handle, num_tokens_per_rank, is_token_in_rank, num_tokens_per_expert, None, None, expert_alignment)
    else:
        recv_x, recv_topk_idx, recv_topk_weights, num_recv_tokens_per_expert_list, handle = ts_buffer.dispatch(x, None, num_tokens_per_rank, is_token_in_rank, num_tokens_per_expert, topk_idx, topk_weights, expert_alignment)
    ...
    if not cached_dispatch:
        assert torch.equal(recv_topk_idx, ref_recv_topk_idx), f'[rank {rank}] recv_topk_idx mismatch, max err: {(recv_topk_idx - ref_recv_topk_idx).abs().max()}'
        assert torch.equal(recv_topk_weights, ref_recv_topk_weights), f'[rank {rank}] recv_topk_weights mismatch, max err: {(recv_topk_weights - ref_recv_topk_weights).abs().max()}'
        ...
        assert torch.equal(rank_prefix_matrix, ref_rank_prefix_matrix), f'[rank {rank}] rank_prefix_matrix mismatch, max err: {(rank_prefix_matrix - ref_rank_prefix_matrix).abs().max()}'
        assert torch.equal(channel_prefix_matrix, ref_channel_prefix_matrix), f'[rank {rank}] channel_prefix_matrix mismatch, max err: {(channel_prefix_matrix - ref_channel_prefix_matrix).abs().max()}'
        assert torch.equal(recv_channel_prefix_matrix, ref_recv_channel_prefix_matrix), f'[rank {rank}] recv_channel_prefix_matrix mismatch, max err: {(recv_channel_prefix_matrix - ref_recv_channel_prefix_matrix).abs().max()}'
        assert torch.equal(recv_src_idx, ref_recv_src_idx), f'[rank {rank}] recv_src_idx mismatch, max err: {(recv_src_idx - ref_recv_src_idx).abs().max()}'
        assert torch.equal(is_token_in_rank, ref_is_token_in_rank), f'[rank {rank}] is_token_in_rank mismatch, max err: {(is_token_in_rank - ref_is_token_in_rank).abs().max()}'
        assert torch.equal(send_head, ref_send_head), f'[rank {rank}] send_head mismatch, max err: {(send_head - ref_send_head).abs().max()}'
    ...
    assert torch.equal(combined_x, ref_combined_x), f'[rank {rank}] combined_x mismatch, max err: {(combined_x - ref_combined_x).abs().max()}'
    assert torch.equal(combined_topk_weights, ref_combined_topk_weights), f'[rank {rank}] combined_topk_weights mismatch, max err: {(combined_topk_weights - ref_combined_topk_weights).abs().max()}'

    group.barrier()
    if rank == 0:
        print(f'Check passed for combine. ✅')
