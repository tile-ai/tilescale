...
def gemm_kernel(
    ...
    def main(
        ...
        scatter_signal_buf: T.Tensor((num_local_rank), T.uint32),
        counter_signal_buf: T.Tensor((num_local_rank), T.uint32),
        ...
        with T.Kernel(T.ceildiv(M, block_M) * T.ceildiv(N, block_N), threads=threads) as (bid):
            ...
            if segment <= segment_end:
                ...
                if T.Cast(T.int32, val[0]) == num_pid_n * tiled_m_size - 1:
