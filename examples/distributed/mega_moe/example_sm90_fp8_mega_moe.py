    ...
    def main(
        ...
        with T.Kernel(num_sms, threads=threads) as bid:
            ...
            if bid == 0:
                ...
                if tx < route_threads:
                    for count_local_expert in T.serial(tx, num_experts_per_rank, route_threads):
            ...
            if tx < dispatch_threads:
                ...
                for pull_idx in T.serial(bid * dispatch_warps + dispatch_warp, num_experts_per_rank * capacity, num_sms * dispatch_warps):
            ...
            if tx >= producer_begin and tx < producer_end:
                ...
                for producer_schedule_step in T.serial(num_expert_waves * max_tile_rounds_per_expert_wave):
                    producer_expert_wave = producer_schedule_step // max_tile_rounds_per_expert_wave
                    producer_tile_round = producer_schedule_step % max_tile_rounds_per_expert_wave
                    ...
                    for producer_wave_expert_offset in T.serial(num_experts_per_wave):
                        producer_wave_expert = producer_wave_begin + producer_wave_expert_offset
                        producer_wave_num_tiles += T.ceildiv(T.min(recv_counts[producer_wave_expert], capacity), block_m) * num_n_blocks

                    if producer_wave_tile < producer_wave_num_tiles:
                        producer_tile_offset = T.alloc_var(T.int32, init=producer_wave_tile)
                        producer_expert = T.alloc_var(T.int32, init=-1)
                        producer_m = T.alloc_var(T.int32, init=0)
                        producer_n = T.alloc_var(T.int32, init=0)
                        for producer_wave_expert_offset in T.serial(num_experts_per_wave):
                            producer_wave_expert = producer_wave_begin + producer_wave_expert_offset
                            producer_expert_m_blocks = T.ceildiv(T.min(recv_counts[producer_wave_expert], capacity), block_m)
                            producer_expert_tiles = producer_expert_m_blocks * num_n_blocks
                    ...
                    if producer_tile_round == max_tile_rounds_per_expert_wave - 1 and producer_wave_num_tiles > 0:
                ...
                for consumer_schedule_step in T.serial(num_expert_waves * max_tile_rounds_per_expert_wave):
                    consumer_expert_wave = consumer_schedule_step // max_tile_rounds_per_expert_wave
                    consumer_tile_round = consumer_schedule_step % max_tile_rounds_per_expert_wave
                    ...
                    for consumer_wave_expert_offset in T.serial(num_experts_per_wave):
                        consumer_wave_expert = consumer_wave_begin + consumer_wave_expert_offset
                        consumer_wave_num_tiles += T.ceildiv(T.min(recv_counts[consumer_wave_expert], capacity), block_m) * num_n_blocks
                    ...
                    if consumer_wave_tile < consumer_wave_num_tiles:
                        ...
                        for consumer_wave_expert_offset in T.serial(num_experts_per_wave):
                            consumer_wave_expert = consumer_wave_begin + consumer_wave_expert_offset
                            consumer_expert_m_blocks = T.ceildiv(T.min(recv_counts[consumer_wave_expert], capacity), block_m)
                            consumer_expert_tiles = consumer_expert_m_blocks * num_n_blocks
                        ...
                        if consumer_expert >= 0 and consumer_n * block_n < l1_n:
                            ...
                            for consumer_k in T.serial(num_k_blocks):
                                ...
                                T.gemm(a_shared[consumer_stage, :, :], b_shared[consumer_stage, :, :], partial, transpose_B=True)
                                ...
                                for i, j in T.Parallel(block_m, block_n):
                                    accum[i, j] = T.cast(partial[i, j], T.bfloat16) * T.cast(
                                        act_scale[i] * weight_scale[2 * (j // (2 * SCALE_GRANULARITY)) + (j % 16) // 8], T.bfloat16
                                    ) + accum[i, j]
                                T.clear(partial)
                    ...
                    if consumer_tile_round == max_tile_rounds_per_expert_wave - 1 and consumer_wave_num_tiles > 0:
                    ...
                    if consumer_wave_tile < consumer_wave_num_tiles:
                        ...
                        if (
                            ...
                            for consumer_k in T.serial(num_k_blocks):
                                ...
                                T.mbarrier_arrive(stage_barriers[pipeline_stages + consumer_stage])
                            consumer_step += num_k_blocks
                            # Map the SM90 M64 WGMMA fragment owners directly to
                            # packed BF16 remote stores, without a shared-memory epilogue.
...
def main(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    def run_pipeline(check_capacity: bool = False):
        fused_l1(
            ...
            recv_x, recv_x_sf, recv_weights, src_ranks, src_tokens, src_topk,
        ...
        fused_l2(
            l2_x, l2_fp8, l2_x_sf, l2_sf, recv_counts, src_ranks,
