...
def torch_reverse_all_to_all_transpose_reference(data_src, group):
    ...
    for pe_idx in range(world_size):
        recv_data = torch.empty(
            batch_size, heads_per_pe, seq_per_pe, head_dim,
            dtype=data_src.dtype, device=data_src.device
        )
    ...
    result = torch.empty(
        batch_size, seq_per_pe, num_heads, head_dim,
        dtype=data_src.dtype, device=data_src.device
    )
...
def test_reverse_transpose_all_to_all_with_golden_reference():
    ...
    func = sequence_parallel_reverse_all_to_all_transpose(
        PE_num, args.batch_size, args.num_heads, args.seq_len, args.head_dim, args.dtype
    )
