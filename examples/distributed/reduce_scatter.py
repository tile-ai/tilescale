...
def create_reduce_scater_2d_ctx(max_M, N, rank, world_size, local_world_size, dtype, allocator, overlap_with_gemm=True,
    ...
    ctx = ReduceScatter2DContext(max_M=max_M, N=N, rank=rank, world_size=world_size, local_world_size=local_world_size,
                                 ...
                                 barrier=symm_barriers, reduction_stream=reduction_stream,
...
def kernel_ring_reduce_tma(
    ...
    def _kernel_ring_reduce_tma(
        ...
        with T.Kernel(T.ceildiv(M_per_rank, block_M), T.ceildiv(N, block_N),
...
def intra_node_scatter(input_intra_node, scatter_bufs_intra_node: List[torch.Tensor],
    ...
    for i in range(local_world_size):
        ...
        src = input_intra_node[
            remote_local_rank * M_per_rank : (remote_local_rank + 1) * M_per_rank, :
        ]
        dst = scatter_bufs_intra_node[remote_local_rank][
...
def reduce_scatter_for_each_node(input: torch.Tensor, ctx: ReduceScatter2DContext,
    ...
    print(f"reduce_scatter_for_each_node: M={M}, N={N}, M_per_rank={M_per_rank}, M_per_node={M_per_node}, nnodes={nnodes}, node_id={node_id}, local_rank={local_rank}")
