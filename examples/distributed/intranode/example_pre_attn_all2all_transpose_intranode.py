"""
Intranode pre-attention all-to-all (transpose) using tilescale IPC API.

Input:  [B, S_PE, NH, D]  — partial sequence, full heads per rank
Output: [B, H_PE, S,   D] — partial heads, full sequence per rank

Rank r sends src[:, :, p*H_PE:(p+1)*H_PE, :] (shape [B, S_PE, H_PE, D])
...
def torch_reference(src, group, H_PE, S_PE):
    ...
    # send [B, S_PE, H_PE, D] to each rank
    input_list = [src[:, :, p * H_PE : (p + 1) * H_PE, :].contiguous() for p in range(PE_num)]
    output_list = [torch.empty(B, S_PE, H_PE, D, dtype=src.dtype, device=src.device) for _ in range(PE_num)]
    dist.all_to_all(output_list, input_list, group=group)

    S = S_PE * PE_num
    result = torch.empty(B, H_PE, S, D, dtype=src.dtype, device=src.device)
    for r in range(PE_num):
        # output_list[r] is [B, S_PE, H_PE, D] from rank r; transpose to [B, H_PE, S_PE, D]
        result[:, :, r * S_PE : (r + 1) * S_PE, :] = output_list[r].transpose(1, 2)
    return result


def kernel_pre_attn_all2all_transpose(PE_num, B, NH, S_PE, D, dtype="float16"):
    H_PE = NH // PE_num
    S = S_PE * PE_num
    NUM_BLOCKS_X = B * H_PE
    ...
    @T.prim_func
    def main(
        data_src: T.Tensor((B, S_PE, NH, D), dtype),
        ...
        with T.Kernel(NUM_BLOCKS_X, PE_num, threads=128) as (bx, target_pe):
            ...
            batch_idx = bx // H_PE
            head_idx = bx % H_PE
            src_head_idx = target_pe * H_PE + head_idx

            for seq_idx in T.serial(S_PE):
                T.put_block(
                    src=T.address_of(data_src[batch_idx, seq_idx, src_head_idx, 0]),
                    dst=T.address_of(data_dst[batch_idx, head_idx, rank[0] * S_PE + seq_idx, 0]),
                    size=D,
                    dst_pe=target_pe,
                )
...
def main(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    src_bufs = tilelang.tensor((B, S_PE, NH, D), dtype, allocator=allocator, return_peers=True)
    dst_bufs = tilelang.tensor((B, H_PE, S, D), dtype, allocator=allocator, return_peers=True)
