"""
Intranode pre-attention all-to-all using tilescale IPC API.
...
def torch_reference(src, group, H_PE, S_PE):
    ...
    for r in range(PE_num):
        result[:, :, r * S_PE : (r + 1) * S_PE, :] = output_list[r]
    return result


def kernel_pre_attn_all2all(PE_num, B, NH, S_PE, D, dtype="float16"):
    H_PE = NH // PE_num
    S = S_PE * PE_num
    NUM_BLOCKS_X = B * H_PE
    ...
    @T.prim_func
    def main(
        data_src: T.Tensor((B, NH, S_PE, D), dtype),
        ...
        with T.Kernel(NUM_BLOCKS_X, PE_num, threads=128) as (bx, target_pe):
            ...
            T.put_block(
                src=T.address_of(data_src[batch_idx, src_head_idx, 0, 0]),
                dst=T.address_of(data_dst[batch_idx, head_idx, rank[0] * S_PE, 0]),
                size=S_PE * D,
                dst_pe=target_pe,
            )
...
def main(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    src_bufs = tilelang.tensor((B, NH, S_PE, D), dtype, allocator=allocator, return_peers=True)
