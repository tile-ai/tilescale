...
class EPException : public std::exception {
  std::string message;
public:
  EPException(const char* name, const char* file, int line, const std::string& error) {
...
#define EP_HOST_ASSERT(cond) do { if (!(cond)) { throw EPException("Assertion", __FILE__, __LINE__, #cond); } } while (0)
#endif

#ifndef CUDA_CHECK
#define CUDA_CHECK(cmd) do { cudaError_t e = (cmd); if (e != cudaSuccess) { throw EPException("CUDA", __FILE__, __LINE__, cudaGetErrorString(e)); } } while (0)
#endif

static size_t numel_of(const std::vector<int64_t> &shape) {
  return std::accumulate(shape.begin(), shape.end(), size_t{1},
    [](size_t a, int64_t b){ if (b < 0) throw std::runtime_error("Negative dim"); return a * (size_t)b; });
}
...
torch::Tensor create_tensor(const std::vector<int64_t> &shape, c10::ScalarType dtype) {
  auto current_device = c10::cuda::current_device();
  auto options = at::TensorOptions(at::kCUDA).dtype(dtype).device_index(current_device);
