...
static int64_t safe_mul_int64(int64_t a, int64_t b) {
  ...
  if (a > maxv / b) throw std::overflow_error("integer overflow in multiplication");
  return a * b;
}
...
static at::ScalarType dtype_from_string(const std::string &s) {
  ...
  if (s == "uint32") return at::kUInt32;
  if (s == "uint64") return at::kUInt64;
...
torch::Tensor tensor_from_ptr(uint64_t ptr_val, std::vector<int64_t> shape,
                              ...
                              int64_t device, bool take_ownership) {
  if (ptr_val == 0) throw std::runtime_error("Received null pointer (0).");
