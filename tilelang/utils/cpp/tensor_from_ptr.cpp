...
static at::ScalarType dtype_from_string(const std::string& s) {
    ...
    if (s == "bfloat16" || s == "bfloat") return at::kBFloat16;
    if (s == "float64" || s == "double") return at::kDouble;
    if (s == "int32" || s == "int") return at::kInt;
    if (s == "int64" || s == "long") return at::kLong;
    if (s == "uint8" || s == "byte") return at::kByte;
...
static int64_t element_size_bytes(at::ScalarType st) {
    switch (st) {
        ...
        case at::kDouble: return 8;
        case at::kInt: return 4;
        case at::kLong: return 8;
        case at::kByte: return 1;
        case at::kChar: return 1;
        case at::kBool: return 1;
        default:
            throw std::runtime_error("Unsupported scalar type for element size calculation");
    }
}
...
torch::Tensor tensor_from_ptr(uint64_t ptr_val,
    ...
    if (take_ownership) {
        ...
        deleter = [saved_ptr](void* /*unused*/) {
            ...
            if (cerr != cudaSuccess) {
                std::fprintf(stderr, "tensor_from_ptr: cudaFree failed in deleter: %s\n", cudaGetErrorString(cerr));
            }
        };
    } else {
        deleter = [](void*) { /* no-op */ };
    ...
    if (nelems == 0) {
        ...
        // Note: from_blob expects a pointer to memory of correct device when device is CUDA.
...
PYBIND11_MODULE(alloc_cuda, m) {
    m.doc() = "Utility to wrap a CUDA device pointer (uintptr_t) into a torch.Tensor (zero-copy)";
    m.def("tensor_from_ptr", &tensor_from_ptr,
          ...
          "Create a torch.Tensor from an external CUDA device pointer (uintptr_t).\n\n"
          ...
          "take_ownership: if true the tensor's deleter will call cudaFree on ptr when freed");
}
