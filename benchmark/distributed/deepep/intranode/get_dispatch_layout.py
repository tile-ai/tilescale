    ...
    def main(
        ...
        with T.Kernel(num_sms, threads=threads) as bid:
            ...
            if expert_begin_idx[0] < expert_end_idx[0]:
                for i in T.serial(0, T.ceildiv(num_tokens-tid, threads)):  # tl does not support strided loop
                    for j in T.serial(0, num_topk):
                        expert_idx = T.alloc_local([1], "int32")
                        expert_idx[0] = T.cast(topk_idx[tid + i * threads, j], "int32")
                        if expert_begin_idx[0] <= expert_idx[0] and expert_idx[0] < expert_end_idx[0]:
                            tokens_per_expert_per_thread[tid, expert_idx[0] - expert_begin_idx[0]] += 1
...
def test_get_dispatch_layout(
    ...
    buffer = deep_ep_cpp.Buffer(
        0, num_ranks, 0, 0, False, False
    )
    deepep_impl = lambda: buffer.get_dispatch_layout(
        topk_idx,
        num_experts,
        None,
        False,
    ...
    tl_impl = lambda: get_dispatch_layout(
        topk_idx,
        num_experts,
        num_ranks
    )
